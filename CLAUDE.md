...
Pages: `/` · `/auth/signin` · `/auth/signup` · `/dashboard` · `/profile` · `/settings` · `/team` (route groups `(auth)`, `(dashboard)`). After sign-in, users land on `/team`. Backend: `GET /api/health` (public); everything else under `/api` requires `Authorization: Bearer <ID token>`.
